...
- **`current_setting('app.tenant_id')` sans le paramètre `missing_ok`.** Si le GUC n'est pas positionné, PostgreSQL **lève une exception** au lieu de retourner NULL. Une requête sans scope ne renvoie donc pas « zéro ligne » : elle **plante**. La différence est capitale — zéro ligne est un bug qui passe la revue et se réveille en production, une exception est un bug qui casse le test dès la première exécution. On veut échouer bruyamment, tôt.
...
| **R2** | **Prisma + RLS**. Pas de support natif. Une extension mal faite = fuite silencieuse. | M | **É** | Prototyper en jour 1 de la phase 1 ; tests §16.4-A ; lint bloquant ; **repli Drizzle décidé avant la fin de la phase 1, pas après**. |
